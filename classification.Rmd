...
df_nyc_food_deserts_shp <- df_nyc_food_deserts_shp %>% 
...
all_cols_1 <- c(5:15,46:59)

# filter df
df_prepped <- df_nyc_food_deserts_shp %>% 
  st_drop_geometry() %>%
  filter(!is.na(median_family_income)) %>% 
  select(all_cols_1)

# create coords 
coords <- df_nyc_food_deserts_shp %>% 
  st_drop_geometry() %>% 
  filter(!is.na(median_family_income)) %>% 
  select(lat,lon)

# model
grf_model <- grf(formula = grf_form,
    dframe = df_prepped,
    bw=162,
    ntree = 500,
    mtry = 2,
...
```{r}
grf_form <- c("lila_tracts_half_and10 ~ pop2010+ohu2010+group_quarters_flag+numgqtrs+pctgqtrs+hunv_flag+low_income_tracts+poverty_rate+median_family_income+tract_lowi+tract_kids+tract_seniors+tract_white+tract_black+tract_asian+tract_nhopi+tract_aian+tract_o_multir+tract_hispanic+tract_hunv+tract_snap")
```
...
```{r group-loc-desc}
df_nypd_shooting %>% st_drop_geometry() %>% 
...
# drop geometry
df_nypd_shooting_prep_2021 <- df_nypd_shooting_2021 %>% sf::st_drop_geometry()

# simple model
glm(formula = formula_nypd, data = df_nypd_shooting_prep_2021,
    family = "binomial")
```
Look at just 2022

```{r}
df_nypd_shooting_2021 <- df_nypd_shooting %>% 
