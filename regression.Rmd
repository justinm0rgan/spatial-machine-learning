...
# define url
url_bridges <- "https://data.cityofnewyork.us/api/views/4yue-vjfc/rows.csv?accessType=DOWNLOAD"

# create tempfile
destfile = tempfile()

# download file
download.file(url = url_bridges, destfile = destfile)

# save as df
df_bridges <- read.csv(destfile) %>% janitor::clean_names()

# convert dependent to factor for multi class classification
df_bridges$verbal_rating <- factor(df_bridges$verbal_rating,
                                      levels = c("CLOSED","POOR","FAIR",
                                                 "GOOD","VGOOD"))
