...
Highest sigficant p-values are `pct_foreign_born` , `median_age` and `pop_density`. The first two are both negatively correlated with `median_value`, therefore as census tracts percentage of foreign born and median age increase, median home value decreases. Much more so for `pct_foreign_born`. It looks like the denser the population, `median_value` increases. Other positvely correlated values are `median_rooms`, `pct_asian`, `median_structure_age` and `pct_white`. Some of the other negatively correlated values are `percent_ooh` and `median_age`. 
...
Assumption of residual independence commonly violated in models using spatial data due to *spatial autocorrelation* in the error term. Which means model performence depends on geographic location. This can be assessed using Moran's I.
